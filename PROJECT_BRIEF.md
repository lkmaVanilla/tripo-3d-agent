...
长期交互围绕同一资产持续进行：一个制作目标完成后，用户可以在原会话中基于已有资产结果提出后续目标，继续与 Agent 协作。例如，在相应能力完成接入后，可先生成角色，再在原会话提出骨骼绑定和动画制作目标。该交互方向见 [ADR 0021](docs/adr/0021-continuous-asset-collaboration.md)，当前连续创作阶段已实现同会话的解释、文本再生成及指定历史版本减面；绑定与动画仍需后续接入。
...
- 第一阶段 MVP 以文本到静态资产的完整制作链路验证 Agent 基础能力；原有验收门槛继续保留。
- 当前连续创作阶段增加聊天工作台、同会话后续目标、显式单版本引用、不可变版本来源与整体保留；能力仍限定在文本生成、静态 GLB 减面和技术解释。
- 图片输入、用户上传已有资产、骨骼绑定和动画仍属后续阶段，不能把 Tripo 已开放的能力当成本项目已接入的能力。
- 一个会话围绕同一资产主题，顺序承载多个独立目标；每次目标使用新的执行身份与预算，澄清和纠偏留在原执行内。引用限定为该会话生产的单个版本。执行或退出处理中只能保留下一条草稿。
...
以下为第一阶段静态资产 MVP 的能力目标，具体评测门槛沿用 CONTEXT.md；后续阶段独立定义能力范围与验收条件。
...
## Continuous Creation Acceptance

当前阶段验收在原标准上增加：同会话生成与历史版本加工、真实父关系、旧版本保留、纯回答零生产、跨执行事件补读及恢复、匿名隔离和整体到期清理。系统回归、84 次真实 LLM 固定供应商评测及真实 Tripo 文件上传加工分别提供证据。实施和验收状态见 [连续创作验证记录](docs/verification/conversation-workspace.md)，不能用单条链路成功代替完整验收。
