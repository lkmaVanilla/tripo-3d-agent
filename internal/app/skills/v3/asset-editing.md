...
仅减面目标：若 input_assessment 显示已有输入满足本目标技术要求，调用 finish_request(outcome="answer") 说明无需加工，零生产、零新版本。明确重新生成目标仍可生成新版本。每轮只调用一个工具或一个 Skill，不能同时加载与加工；结果或限制都要通过 finish_request 收尾。
