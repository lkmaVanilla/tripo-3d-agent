...
import {ConversationConnection} from './workspace-connection.mjs';
...
const connection=new ConversationConnection({load:id=>api(conversationPath(id)),onSnapshot:snapshot=>{const state=currentState(),wasStopping=state&&(state.stopPending||activeRun(state)?.status==='stopped');if(state&&mergeSnapshot(state,snapshot)){state.refreshing=false;if(wasStopping&&state.canSend)notice('本次本地执行已停止；已经提交的远端任务不一定会取消。');render();}},onStatus:text=>{$('connection').textContent=text;},onUnavailable:unavailable});
const chat=new ChatView({container:$('messages'),scroll:$('chat-scroll'),onPreview:selectPreview});
...
function showHome(){navigationGeneration++;connection.close();current=null;history.replaceState(null,'',location.pathname+location.search);$('home').hidden=false;$('workspace').hidden=true;$('title').textContent='从想法开始创作';$('page-eyebrow').textContent='3D CREATION';$('connection').textContent='准备就绪';notice('');renderHome();markListSelection();}
...
function renderHome(){
 $('request').value=homeDraft.text;$('request').readOnly=homeDraft.submitting||homeDraft.uncertain;
 $('submit').disabled=homeDraft.submitting||!config?.ready||!homeDraft.text.trim();
 $('submit').textContent=homeDraft.submitting?'正在提交…':homeDraft.uncertain?'确认原提交':'开始创作 ↑';
 $('home-hint').textContent=homeDraft.uncertain?'尚不能确认是否已经接受。请保持原文，用相同提交身份确认。':'一个会话围绕一个资产，保留每次制作的历史版本。';
}
...
function renderComposer(state){
 ...
 $('message').value=draft.text;$('message').readOnly=draft.submitting||draft.uncertain;
 ...
 $('send').textContent=draft.submitting?'正在提交…':draft.uncertain?'确认原提交':slot.kind==='answer'?'发送回答 ↑':'发送 ↑';
...
$('message-form').onsubmit=async event=>{
 ...
 const submission=startSubmission(slot.draft,payload);state.pendingSlot=slot.key;renderComposer(state);notice('');
