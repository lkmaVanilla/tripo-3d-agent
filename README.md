...
长期交互目标是在同一会话围绕同一资产持续创作，一个制作目标完成后可继续基于已有结果提出后续目标。当前首版仍是一会话一次资产请求，交付后可查看和下载，尚不支持追加制作目标。

**当前 MVP 范围：文本到静态资产。** 当前实现由一个 Agent 澄清、规划、调用 Tripo，并依据实际 GLB 的技术检查结果决定交付、减面、重新生成或停止。图片输入、已有资产导入、骨骼绑定和动画是后续方向，尚未实现。木箱是首条端到端演示案例，阶段验收仍须满足独立的系统回归与 Agent 评测要求。

本次定位调整保留当前 MVP 的范围与原验收标准。后续能力按独立阶段规划，优先级尚未确定；长期目标与阶段划分见 [PROJECT_BRIEF.md](PROJECT_BRIEF.md)。
